""" File to house integration tests for the delta update state pair """

import subprocess
from state_testing_utils import get_exec_command_for_python_program

RUN_IN_FILE_PATH = './tests/state_integration_tests/data/delta_update_test/run_delta_in.sh'
RUN_OUT_FILE_PATH = './tests/state_integration_tests/data/delta_update_test/run_delta_out.sh'
RUN_X_IN_PATH = './tests/state_integration_tests/data/delta_update_test/run_x_delta_in.sh'
RUN_X_OUT_PATH = './tests/state_integration_tests/data/delta_update_test/run_x_delta_out.sh'
RUN_X_PUB_SUB_BUS = './tests/state_integration_tests/data/pub_sub_bus/run_bus.sh'


def test_delta_update_states():
    """
    This is needed to make sure the delta update out state will properly
    update all of the delta update cases.
    """
...
def test_delta_update_states_with_topic_and_xpub_xsub_bus():
    """
